...
function Agendar() {
  ...
  const ready = sel && slot
  const summary = ready ? `${fmtDate(sel)} • ${slot}` : 'Selecione data e horário'
  const heading = servico === 'tosa' ? 'Agende o banho do seu pet' : 'Agende a consulta do seu pet'
  const typeLabel = servico === 'tosa' ? 'O que seu pet vai fazer?' : 'Tipo de atendimento'
  const typeOptions = servico === 'tosa'
    ? ['Banho', 'Tosa higiênica', 'Banho & Tosa completo', 'Coloração de pelos', 'Corte de unhas', 'Banho + Corte de unhas']
    : ['Consulta de rotina', 'Vacinação', 'Retorno', 'Emergência', 'Exames']
  ...
  function confirm() {
    if (!ready) return
    try {
      const rec = { date: fmtDate(sel), slot, type: typeRef.current ? typeRef.current.value : '', ts: Date.now() }
              ...
              <section style={{ padding: '24px 40px 60px', display: 'flex', gap: 26, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  <div style={{ background: '#fff', borderRadius: 26, padding: '24px 26px', boxShadow: '0 12px 32px rgba(0,0,0,.06)' }}>
                    ...
                    <button onClick={confirm} disabled={!ready} style={{ width: '100%', height: 50, marginTop: 18, border: 'none', borderRadius: 14, fontFamily: "'Fredoka', sans-serif", fontWeight: 600, fontSize: 16, background: ready ? '#E8530E' : '#dfe7e8', color: ready ? '#fff' : '#9aa8ab', cursor: ready ? 'pointer' : 'not-allowed', boxShadow: ready ? '0 10px 24px rgba(232,83,14,.3)' : 'none' }}>Confirmar agendamento</button>
                  </div>
                </div>
              </section>
            </Reveal>
