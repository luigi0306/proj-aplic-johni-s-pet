...
function Carrinho() {
  ...
  function logout() { try { localStorage.removeItem('chew_logged_in'); localStorage.removeItem('chew_user') } catch {}; setCart({ ...cart }) }
  ...
  // se ainda não confirmou o login, não mostra nada (evita piscar a tela antes do redirect)
  ...
  return (
    <div style={{ background: '#F4F1EA', minHeight: '100vh', display: 'flex', justifyContent: 'center', fontFamily: "'Nunito', sans-serif" }}>
      <div style={{ width: '100%', maxWidth: 1180, background: '#F4F1EA', position: 'relative' }}>
        ...
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 40px', background: '#16313b', borderRadius: '0 0 26px 26px', boxShadow: '0 6px 18px rgba(0,0,0,.18)' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 22 }}>
                ...
                <div style={{ background: '#fff', borderRadius: 24, padding: '24px 26px', boxShadow: '0 10px 26px rgba(0,0,0,.06)' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {ids.map((id) => (
                      <div key={id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 14, borderRadius: 16, background: '#F8F6F0' }}>
                ...
                <div style={{ background: '#fff', borderRadius: 24, padding: '24px 26px', boxShadow: '0 10px 26px rgba(0,0,0,.06)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
                    {methods.map(([key, label]) => {
                      const on = key === method
                      return <button key={key} onClick={() => setMethod(key)} style={{ flex: 1, height: 44, borderRadius: 12, border: `2px solid ${on ? '#16313b' : '#e3ddd0'}`, background: on ? '#16313b' : '#fff', color: on ? '#fff' : '#16313b', fontFamily: "'Fredoka', sans-serif", fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>{label}</button>
                    })}
                  </div>
