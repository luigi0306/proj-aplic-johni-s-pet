...
CREATE TYPE tipo_etaria AS ENUM ('Filhote', 'Jovem', 'Adulto', 'Idoso');
...
-- ---------------------------------------------------------------------
-- SOLICITAÇÃO DE ADOÇÃO (formulário Adotar.jsx + painel SolicitacoesAdocao.jsx)
-- ---------------------------------------------------------------------
...
-- FUNCIONÁRIO
-- ---------------------------------------------------------------------
CREATE TYPE tipo_cargo AS ENUM ('Gerente', 'Atendente', 'Estoquista', 'Limpeza', 'Groomer', 'Veterinario');
...
-- ---------------------------------------------------------------------
-- AUTENTICAÇÃO DE FUNCIONÁRIO (login do painel interno)
-- Regra atual, batendo com o que LoginFuncionario.jsx já faz: só Gerente,
-- Atendente, Veterinario e Limpeza têm acesso ao painel (cada um vai para
-- uma tela diferente). Estoquista e Groomer não recebem login.
-- A permissão fina de cada tela dentro do painel (ex.: só Gerente mexe em
-- Equipe) é aplicada no backend/front, não aqui — esta trigger é só a
-- trava de "quem pode logar", direto no banco.
-- ---------------------------------------------------------------------
CREATE TABLE usuario_funcionario (
    id_usuario_funcionario SERIAL PRIMARY KEY,
...
CREATE OR REPLACE FUNCTION valida_cargo_login()
RETURNS TRIGGER AS $$
DECLARE
    cargo_atual tipo_cargo;
BEGIN
    SELECT cargo INTO cargo_atual FROM funcionario WHERE id_funcionario = NEW.id_funcionario;
    IF cargo_atual NOT IN ('Gerente', 'Atendente', 'Veterinario', 'Limpeza') THEN
        RAISE EXCEPTION 'Funcionários com cargo "%" não têm acesso ao painel.', cargo_atual;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_valida_cargo_login
BEFORE INSERT OR UPDATE ON usuario_funcionario
FOR EACH ROW
EXECUTE FUNCTION valida_cargo_login();
...
-- ---------------------------------------------------------------------
CREATE TYPE status_pedido_insumo AS ENUM ('Pendente', 'Comprado');

CREATE TABLE pedido_insumo (
    id_pedido  SERIAL PRIMARY KEY,
    id_insumo  INT REFERENCES insumos(id_insumo) NOT NULL,
    quantidade INT NOT NULL,
    status     status_pedido_insumo NOT NULL DEFAULT 'Pendente',
    criado_em  TIMESTAMP NOT NULL DEFAULT now()
);
...
-- DESPESAS GERAIS
-- ---------------------------------------------------------------------
CREATE TYPE categoria_despesa AS ENUM ('Salário', 'Insumos', 'Manutenção', 'Outros');
...
-- ÁREAS DE LIMPEZA
-- ---------------------------------------------------------------------
CREATE TYPE status_limpeza AS ENUM ('Limpo', 'Pendente', 'Urgente');

CREATE TABLE area_limpeza (
    id_area        SERIAL PRIMARY KEY,
    nome           VARCHAR(60) NOT NULL UNIQUE,
    status         status_limpeza NOT NULL DEFAULT 'Pendente',
    atualizado_em  TIMESTAMP NOT NULL DEFAULT now(),
    id_funcionario INT REFERENCES funcionario(id_funcionario) -- quem fez a última atualização
);

-- seed das áreas que já aparecem hardcoded no front hoje
INSERT INTO area_limpeza (nome, status) VALUES
    ('Clínica Veterinária', 'Pendente'),
    ('Loja', 'Limpo'),
    ('Sala de Banho e Tosa', 'Urgente'),
    ('Canil', 'Pendente'),
    ('Gatil', 'Limpo'),
    ('Recepção', 'Pendente'),
    ('Estoque', 'Limpo');
...
CREATE INDEX idx_usuario_funcionario_fk  ON usuario_funcionario(id_funcionario);
CREATE INDEX idx_pedido_insumo_id_insumo ON pedido_insumo(id_insumo);
CREATE INDEX idx_despesa_categoria_data  ON despesa(categoria, data);
CREATE INDEX idx_area_limpeza_status     ON area_limpeza(status);
